...
//        var_dump($param);
//        die();
        $last_message = DB::table('message')->where('phone', '=', trim($param['phone']))
            ->where('delete_time', '<=', '0')
            ->first();
...
//            $data = json_decode(file_get_contents(''));
        ...
        // 启动事务
        DB::beginTransaction();
        $today_message = Db::table('message')->where('phone', '=', trim($param['phone']))->where('delete_time', '<=', '0')->where('update_time', '>=', date("Y-m-d 00:00:00"))->first();
        if(count((array)$today_message) <= 0){
            //数据不存在
            $insert_status = Db::table('message')->insert($msg);
            // 提交事务
            DB::commit();
        }else{
            // 回滚事务
    ...
    public function send_mail($mail,$title,$content,$receive_user){
        try {
            ...
            // $mail->addAddress('zhangxx19940204@163.com');               // 收件人
